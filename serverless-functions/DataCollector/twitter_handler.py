...
class TwitterAPI:
   def __init__(self, settings):
      ...
      self._twitter_api = tweepy.API(
         twitter_auth, wait_on_rate_limit=True
      )
   ...
   def get_trends(self):
      ...
      num = self._trend_num
      return self._twitter_api.get_place_trends(woeid)[0]["trends"][:num]
   ...
   def get_n_tweets_query_iterator(self, query: str, n_pages: int) -> PageIterator:
      ...
      """
      return tweepy \
         .Cursor(self._twitter_api.search_tweets, q=query, count=100) \
         .pages(n_pages)